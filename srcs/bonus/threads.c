...
static void
	render_thread_row(t_thread_data *info, int y, double ci)
{
	int				x;
	double			cr;
	double			it;
...
static double
	get_ci(t_thread_data *info, int y)
{
...
static void
	dispatch_simd(t_thread_data *info, int y, double ci)
{
...
void
	*thread_render_generic(void *arg)
{
	t_thread_data	*info;
	int				y;
	double			ci;
...
int
	run_threaded_render(t_data *data)
{
	...
	if (num > MAX_THREADS)
		num = MAX_THREADS;
	step[0] = (data->math.max_r - data->math.min_r)

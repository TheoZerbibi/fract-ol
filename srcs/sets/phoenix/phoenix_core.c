...
static double phoenix_smooth(int iter, double *sq, t_data *data) {
...
static void phoenix_step(double *z, double *prev, double *sq, t_data *data) {
  double tmp[2];
...
static double phoenix_loop(t_data *data, double cr, double ci) {
  double z[2];
  double prev[2];
  double sq[2];
  int i;
...
double is_phoenix(t_data *data, double cr, double ci) {
  return (phoenix_loop(data, cr, ci));
}
...
void init_phoenix(t_data *data) {
  ...
  data->fractal.iterate = &is_phoenix;
}
